...
/**
 * index.php — home: herói, catálogo, como encomendar e unidades.
 */
...
// Matriz primeiro na listagem de unidades.
...
<section id="catalogo" class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
  ...
  <!-- Busca + filtros -->
  <div class="sticky top-[4.5rem] z-30 -mx-4 mt-8 border-y border-base-300 bg-base-200/90 px-4 py-4 backdrop-blur-md sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
    ...
    <div class="flex flex-col gap-4">
      <div class="relative">
        ...
        <input id="busca" type="search" data-busca-input
               class="input h-12 w-full rounded-2xl border-campo bg-papel pl-12 pr-4 text-base shadow-bandeja"
               placeholder="Buscar por coxinha, cupcake, bebida…"
               autocomplete="off"
               aria-describedby="contagem-catalogo">
      </div>

      <div class="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-6 sm:gap-y-3">
        <!-- Encomendas x Balcão: derivado da tag do produto -->
        <div class="sem-barra -mx-4 flex gap-2 overflow-x-auto px-4 sm:mx-0 sm:flex-wrap sm:overflow-visible sm:px-0" role="group" aria-label="Tipo de venda">
          <button type="button" data-filtro-linha="" aria-pressed="true"
                  class="chip">Tudo</button>
          <button type="button" data-filtro-linha="atacado" aria-pressed="false"
                  class="chip">Encomendas</button>
          <button type="button" data-filtro-linha="varejo" aria-pressed="false"
                  class="chip">Balcão</button>
        </div>

        <div class="hidden h-6 w-px bg-base-300 sm:block" aria-hidden="true"></div>
      ...
      <p id="contagem-catalogo" class="text-sm font-semibold text-crust" data-contagem aria-live="polite">
        <?= e((string) count($produtos)) ?> itens no catálogo
      </p>
    </div>
  </div>
...
<section id="encomendas" class="border-y border-base-300 bg-base-200">
  <div class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
    ...
    <ol class="mt-10 grid gap-6 md:grid-cols-3">
      ...
      $passos = [
          ...
          [
              ...
              'texto'  => 'No topo da página, diga qual loja vai atender você. É ela que recebe o pedido e manda o catálogo em PDF.',
          ],
          [
    ...
    <div class="mt-10 flex flex-col gap-5 rounded-bandeja bg-neutral p-7 text-neutral-content sm:flex-row sm:items-center sm:p-9">
      ...
      <a href="#unidades" class="btn h-12 min-h-12 shrink-0 border-none bg-accent px-6 font-bold text-accent-content hover:bg-accent/85">
...
<!-- ============================================================
     UNIDADES
     ============================================================ -->
<section id="unidades" class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
  <div class="max-w-2xl">
    <h2 class="text-3xl sm:text-4xl">Nossas unidades</h2>
    <p class="mt-3 text-lg leading-relaxed text-crust">
      A matriz e mais <?= e((string) (count($unidades) - 1)) ?> lojas. Escolha a sua para receber o catálogo em PDF
      e falar direto com quem vai preparar o pedido.
    </p>
  </div>
